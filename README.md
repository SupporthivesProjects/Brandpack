# Simseekr

Start Date : 17th april
End Date : 25th april

QA - Parth


Developer Team:Rahul,Dhiraj,Trithak,Medhanash,Saakshi,Saakshi K,<br>
Project Report : Jeet <br>
Team Lead : Vijay<br>

Vijay:Header,Footer,<br>
Rahul: home page, checkout page, checkout-success, checkout-error same page (pop)<br>
Dhiraj: Plan-page,Plan-individual,Plans-Regional, Plan-individual-Regional, same page (pop)<br>
Trithak: Aboutus-page,Account page all, same page (pop)<br>
Medhnash:Privacy,Terms,Compatibility,error page, same page (pop)<br>
saakshi c : login, reset password,reset password request  ,register,register(error) ,404 error , same page (pop)<br>
saakshi k : help-center, contactus, contactus-succesful, contactus-error, same page (pop)<br>


figma url : https://www.figma.com/design/690IrqqDz13qloEkNWhADo/AD534---Reskin--E-Sim----Simseekr.com?node-id=11206-110278&t=69h8ozdvRWavGc1x-0


prototype link :https://www.figma.com/proto/690IrqqDz13qloEkNWhADo/AD534---Reskin--E-Sim----Simseekr.com?node-id=8256-2700&t=69h8ozdvRWavGc1x-0&scaling=min-zoom&content-scaling=fixed&page-id=3357%3A851

